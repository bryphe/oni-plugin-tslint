...
const activate = (Oni) => {
    ...
    const doLintForFile = async (args) => {
        ...
        const currentWorkingDirectory = getCurrentWorkingDirectory(args.bufferFullPath)
        ...
        const errors = await executeTsLint(filePath, [args.bufferFullPath], currentWorkingDirectory)

        // When running for a single file, only the filename will be included in the results
        const fileName = path.basename(args.bufferFullPath)

        const fileErrors = errors[fileName] || []

        Oni.diagnostics.setErrors("tslint-ts", args.bufferFullPath, fileErrors, "yellow")
    ...
    // TODO: Replace with onBufferSaved event...
